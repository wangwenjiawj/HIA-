...
# VAL_POSE_ANNO_FILE = './LIP/TrainVal_pose_annotations/LIP_SP_VAL_annotations.json'

DATA_DIRECTORY = '/media/qd/copy/wangwenjia/master/CorrPM-master/LIP/images_labels'
DATA_LIST_PATH = './train_id.txt'
VAL_POSE_ANNO_FILE = './LIP/TrainVal_pose_annotations/LIP_SP_TRAIN_annotations.json'

IGNORE_LABEL = 255
NUM_CLASSES = 20
SNAPSHOT_DIR = './snapshots/'
INPUT_SIZE = (473, 473)
RESTORE_FORM = '/media/qd/copy/wangwenjia/master/CorrPM-master/snapshots/HRNet+HMA(seg+seg)/LIP_best.pth'
...
def get_arguments():
    ...
    parser.add_argument("--pose-anno-file", type=str, default=VAL_POSE_ANNO_FILE,
    ...
    parser.add_argument("--num-points", type=int, help='Num of class of keypoint', default=16)
    parser.add_argument("--restore-from", type=str, default=RESTORE_FORM,
...
def valid(model, valloader, input_size, num_samples, gpus):
    ...
    with torch.no_grad():
        for index, batch in enumerate(valloader):
            ...
            outputs = model(image.cuda())#image(1,3,256,256)
            ...
            if gpus > 1:
                ...
                parsing = outputs[0][1]#(1,20,64,64)
...
def main():
    ...
    transform = transforms.Compose([
        ...
        transforms.Resize((256, 256), 3),###############512
        transforms.ToTensor(),
        normalize,
    ])
